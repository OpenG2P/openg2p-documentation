---
description: The following technologies and tools are used by Social Registry
---

# Technology Stack

## Technology Stack

| Technology                                  | Purpose                 | Version | License                                                           |
| ------------------------------------------- | ----------------------- | ------- | ----------------------------------------------------------------- |
| Ubuntu Server                               | Operating System        | 20.04   | Free                                                              |
| Odoo                                        | Business Apps           | 17.0    | LGPL                                                              |
| Python                                      | Development             | 3.5+    | [PSF License](https://docs.python.org/3/license.html#psf-license) |
| FastAPI                                     | REST API                |         | MIT                                                               |
| Postgres                                    | Database                |         | Postgres License BSD 2-clause "Simplified License"                |
| eSignet                                     | OIDC Provider           |         | MPL 2.0                                                           |
| OpenSearch                                  | Analytics               |         | Apache 2.0                                                        |
| Apache Superset                             | Reporting               |         | Apache 2.0                                                        |
| Debezium                                    | Data streaming          |         | Apache 2.0                                                        |
| ODK                                         | Offline data collection |         | Apache 2.0                                                        |
| MinIO                                       | S3 Storage              |         | AGPL 2.0                                                          |
| Github                                      | Source code management  |         | Commercial (OpenG2P uses Free plan)                               |
| FormIO Builder                              | Forms                   |         | OSL 3.0                                                           |
| FormIO Renderer                             | Forms                   |         | MIT                                                               |
| Docker                                      | Deployment              |         | Apache 2.0                                                        |
| Rancher                                     | Deployment              |         | Apache 2.0                                                        |
| Kubernetes                                  | Deployment              |         | Apache 2.0                                                        |
| Prometheus                                  | Monitoring              |         | Apache 2.0                                                        |
| Grafana                                     | Monitoring              |         | AGPL 3.0                                                          |
| Kafka                                       | Queuing                 |         | Apache 2.0                                                        |
| Nginx                                       | Web proxy               |         | BSD                                                               |
| [ix.ai SMTP](https://gitlab.com/ix.ai/smtp) | Email Service           |         | MIT License                                                       |
| MOSIP ID Generator                          | ID Generation           |         | MPL 2.0                                                           |
