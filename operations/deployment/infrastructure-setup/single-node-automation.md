...
| **User-provided**           | `provided`    | Bring your own certs. Set paths in `tls.rancher_cert` and `tls.rancher_key`.   |
...
Only the Rancher hostname is exposed at the infra level, so a single cert/key pair is needed (a wildcard cert works too).
...
| **DNS**      | Not needed (dnsmasq handles it)                   | A record for the Rancher hostname          |
...
* Validates the DNS A record for the new Rancher hostname
* Obtains Let's Encrypt certificates (Rancher, plus a wildcard per environment)
* Updates Nginx, the Rancher server-url, and per-environment Istio Gateways
