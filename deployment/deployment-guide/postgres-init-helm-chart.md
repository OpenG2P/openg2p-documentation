...
<table><thead><tr><th width="100">Helm Chart Version</th><th width="100">Image version</th><th width="100">Last modified</th><th>Contents</th></tr></thead><tbody><tr><td><a href="https://github.com/OpenG2P/postgres-init/tree/develop">0.0.0-develop</a></td><td>develop</td><td>14-Jun-2026</td><td><p><strong>Development version</strong></p><ul><li>Job is now a Helm pre-install/pre-upgrade hook with <code>helm.sh/hook-delete-policy: before-hook-creation</code>. Fixes the "jobs.batch &#x3C;name> already exists" install failure when a leftover Job from a previous attempt is still in the namespace. See <a href="https://openg2p.atlassian.net/browse/G2P-5127">G2P-5127</a>.</li><li>Added GitHub Actions workflows to build &#x26; push the Docker image and publish the Helm chart.</li><li>Fixed issue with overly long generated Job name.</li></ul></td></tr><tr><td><a href="https://github.com/OpenG2P/postgres-init/tree/v1.2.0">1.2.0</a></td><td>1.2.0</td><td>23-Jun-2026</td><td><p><strong>Stable version</strong></p><ul><li>Job name truncated to ≤63 chars (k8s <code>job-name</code> label limit). See <a href="https://openg2p.atlassian.net/browse/G2P-5186">G2P-5186</a>.</li><li><code>init-db.sh</code> now re-syncs the DB user password on every run → idempotent; fixes auth failures after secret regeneration.</li><li>Job kept as a <strong>regular resource</strong> (not a Helm hook) — avoids deadlock when PostgreSQL is in the same release.</li><li>CI workflows for Docker build &#x26; Helm publish.</li></ul></td></tr><tr><td><a href="https://github.com/OpenG2P/postgres-init/tree/v1.1.0">1.1.0</a></td><td>1.1.0</td><td>23-Jan-2026</td><td><p><strong>Stable version</strong></p><ul><li>Added feature to optionally install Postgres extensions like pg_trgm .</li><li>Bug related to running as an upgrade fixed (in secrets.yaml)</li></ul></td></tr><tr><td><a href="https://github.com/OpenG2P/postgres-init/tree/v1.0.0">1.0.0</a></td><td>1.0.0</td><td>09-Jan-2026</td><td><p><strong>Stable version</strong> with following base features:</p><ul><li>Creation of one or more databases (DB) on an existing Postgres server</li><li>Creation of DB user</li><li>Creation of DB user secret with password</li></ul></td></tr></tbody></table>
