...
{% hint style="warning" %}
**This is a legacy / alternative pattern — it does not match the current OpenG2P production architecture.** The supported model fronts **all** traffic through the single **Nginx reverse proxy** on the Reverse-Proxy node: citizen-facing services on the public channel and admin/app UIs over the Wireguard VPN, all terminated with **customer-supplied TLS certs** at Nginx. It does **not** expose per-environment AWS load balancers straight onto the cluster NodePorts, and PostgreSQL is a **host install on the Storage node** (not reached via an internal NLB). Use this page only if you have a deliberate reason to bypass the reverse proxy; for the standard flow see [Production → Infrastructure Automation](../../../operations/deployment/infrastructure-setup/production-automation/) and [Private Access Channel](../../../operations/deployment/deployment-guide/private-access-channel.md).
{% endhint %}
