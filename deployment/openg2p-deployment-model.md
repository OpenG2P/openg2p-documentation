...
The same production architecture, scaled out for high availability — **more nodes, not a different design**. The two channels remain (private over Wireguard, public for citizen-facing); Rancher and Keycloak continue to run in-cluster (no separate management cluster).
