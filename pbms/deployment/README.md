...
The instructions here pertain to the deployment of all Social Registry and associated components on the Kubernetes cluster using [Helm charts](../../social-registry/developer-zone/registry-helm-chart-3.x.x.md). All the components are installed in the same namespace. The deployment may be achieved by the following methods:
...
10. Go through each app's configuration page, and configure the following:
    ...
    4. In the **Registry Settings** section:
...
11. To pull docker from a private repository on Docker Hub, follow guide [here](../../deployment/deployment-guide/pulling-docker-from-private-repository-on-docker-hub.md). \
    **Note:** This step is required only if  have separate private docker image to be deployed or else you can go with default one skip this step.
...
* For PBMS to be able to access Keymanager APIs, create a realm role in Keycloak with the name "KEYMANAGER\_ADMIN" and assign this as a service account role to the Social Registry Keycloak client.
