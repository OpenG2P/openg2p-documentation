...
| -------------------------------------------------------------------------------- | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 4.1.0-develop                                                                    | In progress  | Same as 4.0.0.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | <p>Commons Services 2.0.1<br>IAM Service 1.0.0<br>ID Generator 1.0.0<br>Master Data 0.0.0-develop</p>                                                                                                             | <p><strong>In progress.</strong></p><ul><li>Switched intra-cluster service-to-service calls to internal Kubernetes service DNS for ID Generator, MinIO, and Staff Portal API (no longer routed through ingress).</li><li>Shortened PostgreSQL database / user names to <code>reg_&lt;hash&gt;</code> and <code>idgen_&lt;hash&gt;</code> (8-char SHA-256 of release name) to stay within the 63-char Kubernetes label-value limit on Job names — supports multi-registry installs in the same Postgres. <strong>Breaking</strong> for upgrades from 4.0.x; override <code>global.registryDB</code> / <code>global.idGeneratorDB</code> to preserve old names.</li><li>Added release-name length validator (max 30 chars) — fails <code>helm install</code> upfront with a clear message instead of a cryptic Kubernetes error mid-install.</li></ul> |
...
Kubernetes enforces a **63-character limit on label values**. The release name is embedded in the names of several resources created by this chart and its subcharts — and Kubernetes auto-injects the Job name as a Pod label (`batch.kubernetes.io/job-name`). With our naming pattern, release names longer than **30 characters** produce Job names that exceed this 63-character ceiling, causing `helm install` to fail.

{% hint style="warning" %}
**Maximum supported release name length: 30 characters.** The chart contains a built-in validator that fails `helm install` upfront with a clear error message if a longer release name is used.
{% endhint %}
...
### PostgreSQL DB / user naming

Starting with **4.1.0-develop**, the chart uses hash-suffixed DB / user names to stay within the 63-character ceiling for arbitrary release names while preserving uniqueness across multiple registries in the same Postgres instance:

| Resource | Pattern | Example (release `farmer-reg`) |
| --- | --- | --- |
| `global.registryDB` | `reg_<sha256(release) \| trunc 8>` | `reg_8e3f17a2` |
| `global.registryDBUser` | `reg_<hash>_user` | `reg_8e3f17a2_user` |
| `global.idGeneratorDB` | `idgen_<hash>` | `idgen_8e3f17a2` |
| `global.idGeneratorDBUser` | `idgen_<hash>_user` | `idgen_8e3f17a2_user` |

Two different release names → different SHA hashes → different DBs (no collisions).

{% hint style="info" %}
**Upgrading from 4.0.x?** The old DB-name pattern was `<release>` (e.g. `farmer_reg`). To preserve your existing data when upgrading, override the four `global.*DB*` values in your values file to the old release-derived names. Without that, the upgrade will create fresh empty databases.
{% endhint %}
