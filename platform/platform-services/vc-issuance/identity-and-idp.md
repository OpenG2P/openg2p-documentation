...
### Phase-1 specific: release the `phone_number` claim

The Phase-1 data connector is the custom [Registry Data Connector](registry-data-connector.md),
which binds its SQL lookup from a **configurable token claim**. For phone login it uses
**`phone_number`**, so Logto just needs to **release the `phone_number` claim** in the access
token — there is **no constraint on `sub`** (the connector is not tied to the subject identifier).

The citizen is assumed to map **one-to-one** phone → Registry **functional ID**; absence or an
**inactive** record results in "no eligible credential" (handled in the connector's SQL/view).
