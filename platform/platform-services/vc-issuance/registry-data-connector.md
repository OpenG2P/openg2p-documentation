...
| ------------------------------------------------------------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Source view** (which registry fields, joins, active-only)   | the **registry extension**                           | a `meta_data/vc-views/*.sql` view in the manifestation's extension, **keyed on `internal_record_id`** — **auto-deploys with the models** (the db-seed job applies it after migration)                                                                    |
...
| **Photo** (face on the card + in the QR)                      | **deferred to Phase 2**                              | Not part of Phase 1. When taken up, the photograph comes from the **eSignet KYC** response (the ID system's own image, matching what the beneficiary authenticated against), is downscaled to a ~1–2 KB thumbnail and pushed as the **`face`** claim. See [Phase 2](phase-2-device-wallet.md) |
...
* The plugin is **schema-agnostic** — the table/view name, joins and filters are all inside the configured SQL. So the Registry exposes a **read-only view**, **active-only**, surfacing only the VC columns. The plugin doesn't need to know its name; it's just part of the query string.
* **Identifier is configurable (flow-agnostic):** the plugin binds `:id` from whatever identity claim the issuance carries. **Phase 1 does not use this plugin at all** — the Agent Portal API resolves the record itself (national ID → `foundational_id` → **`internal_record_id`**, which is what the VC view is keyed on) and *pushes* the claims. In a self-service/wallet flow the bound claim could be `phone_number` or the ID-system subject. The mapping is configuration, so the same plugin serves both. No IdP `sub` constraint.
* **Multiple phone numbers per person (wallet flow only):** in the OpenG2P registry an individual may have **more than one** phone number (the `phone_numbers` field is a list). If a wallet flow binds `:id` to a phone number, the view must expose **one row per phone number** (e.g. by expanding the list) so any registered number resolves to the record — each number mapping to exactly one person. This does not arise in Phase 1, where the key is `internal_record_id`.
